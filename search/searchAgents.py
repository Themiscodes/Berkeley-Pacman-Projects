...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        for corner in self.corners:
            ...
            # check if it starts on a corner
        ...
        self.beginningState = ( (self.startingPosition), tuple(visited))
...
def cornersHeuristic(state, problem):
    ...
    countCorners = 0
    minimumDistanceToACorner = 0 
    obstaclesToThatCorner = 0
    for cornerState in state[1]:
        if cornerState[1] == False:
            corner = cornerState[0]
            euclid = ((x - corner[0]) ** 2 + (y - corner[1]) ** 2 ) ** 0.5
            countCorners += 1
            obstacles = 0
            # checking for walls
            if corner == corners[0]:
                ...
                if walls[x+1][y]:
                    obstacles += 1 
            if minimumDistanceToACorner == 0: 
                minimumDistanceToACorner = euclid
                obstaclesToThatCorner = obstacles
            elif minimumDistanceToACorner > euclid and obstacles < obstaclesToThatCorner:
                minimumDistanceToACorner = euclid
                obstaclesToThatCorner = obstacles
    if "cornersExplored" in problem.heuristicInfo:
        if countCorners < problem.heuristicInfo["cornersExplored"]: 
            problem.heuristicInfo["cornersExplored"] = countCorners
        elif countCorners > problem.heuristicInfo["cornersExplored"]:
            return minimumDistanceToACorner*2
    else:
        problem.heuristicInfo["cornersExplored"] = countCorners
    return minimumDistanceToACorner
...
def foodHeuristic(state, problem):
    ...
    for pizzaDot in foodGrid.asList():
    ...
    for pizzaDot in foodGrid.asList():
        countDots += 1
    if "pizzas" in problem.heuristicInfo:
        if countDots < problem.heuristicInfo["pizzas"]:
            problem.heuristicInfo["pizzas"] = countDots 
            problem.heuristicInfo[countDots] = 0
        elif countDots > problem.heuristicInfo["pizzas"]:
            problem.heuristicInfo[countDots] += 1
            return problem.heuristicInfo[countDots] + minimumDistanceToAPizzaDot
    else:
        problem.heuristicInfo["pizzas"] = countDots
        problem.heuristicInfo[countDots] = 0
